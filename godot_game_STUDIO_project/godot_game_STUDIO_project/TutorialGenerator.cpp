...
std::vector<Node2D*> godot::TutotialGenerator::_get_corner_rooms()
{
...
void godot::TutotialGenerator::_rebuild_doors(std::vector<Node2D*> rooms)
{
	for (auto room : rooms)
	{
		...
		if (!(bool)room->call("_adjacent_room_is_null", Vector2(0, 1)))
			room->add_child(down_door->instance());

		if (!(bool)room->call("_adjacent_room_is_null", Vector2(0, -1)))
			room->add_child(up_door->instance());

		if (!(bool)room->call("_adjacent_room_is_null", Vector2(1, 0)))
...
void godot::TutotialGenerator::_rebuild_doors(Node2D* room)
{
	std::vector<Node2D*> old_doors = CustomExtensions::GetChildrenByWordInName(room, "Door");
	for (auto door : old_doors)
		door->queue_free();
...
Node2D* godot::TutotialGenerator::_create_boss_room(std::vector<Node2D*>& cornered_rooms)
{
	Ref<RandomNumberGenerator> rng = RandomNumberGenerator::_new();
	rng->randomize();
...
Node2D* godot::TutotialGenerator::_generate_room_to(Node2D* room_to_build)
{
	Ref<RandomNumberGenerator> rng = RandomNumberGenerator::_new();
	rng->randomize();
